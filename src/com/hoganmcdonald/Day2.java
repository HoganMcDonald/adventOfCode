...
public class Day2 {
    private static int[][] spreadsheet = {{626, 2424, 2593, 139, 2136, 163, 1689, 367, 2235, 125, 2365, 924, 135, 2583, 1425, 2502},
    ...
    public static int solve() {
        ...
        for (int i = 0; i < spreadsheet.length; i++) {
            int highest = spreadsheet[i][0];
            int lowest = spreadsheet[i][0];
            for (int j = 0; j < spreadsheet[i].length; j++) {
                highest = (spreadsheet[i][j] > highest) ? spreadsheet[i][j] : highest;
                lowest = (spreadsheet[i][j] < lowest) ? spreadsheet[i][j] : lowest;
            }
    ...
    private static int evaluateRow(int[] row) {
        int quotient = 0;
        for (int j = 1; j < row.length; j++) {
            for (int k = j - 1; k >= 0; k--) {
                if (row[j] % row[k] == 0) {
                    quotient = row[j] / row[k];
                }
            }
        }
        return quotient;
